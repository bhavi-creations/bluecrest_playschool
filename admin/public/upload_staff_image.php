...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
        ...
        // Generate unique file name and build full path
        $newFileName = uniqid('staff_') . '.' . $ext;
        $uploadPath = $uploadDir . $newFileName; // Full path like: uploads/staff/staff_xyz.jpg

        // Move file to upload folder
        if (move_uploaded_file($fileTmp, $uploadPath)) {
            // Save full path in DB (relative to project root)
            $imagePathToStore = $uploadPath;

            $sql = "INSERT INTO staff_image_uploads (name, role, staff_type, image)
                    VALUES ('$name', '$role', '$staff_type', '$imagePathToStore')";

            if ($conn->query($sql) === TRUE) {
                echo "<script>
                    alert('✅ Staff image uploaded successfully.');
                    window.location.href = 'staff_allimages.php'; // Adjust to actual page
                </script>";
            } else {
                echo "❌ Database Error: " . $conn->error;
            }
