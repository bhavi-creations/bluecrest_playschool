...
<div class="team pb-5">
    <div class="container pb-5">
        ...
        $sql = "SELECT name, role, image FROM staff_image_uploads WHERE staff_type = 'Teacher' ORDER BY id DESC";
        $result = $conn->query($sql);
        ?>
            ...
            <?php if ($result->num_rows > 0): ?>
                <div class="row">
                    ...
                    while ($row = $result->fetch_assoc()):
                        ...
                        // Image URL for browser (don't use file_exists)
                        $imagePath = "admin/public/uploads/staff/" . htmlspecialchars($row['image']);
                        // Fallback image if 'image' field is empty
                        if (empty($row['image'])) {
                            $imagePath = "assets/img/default.png";
                        }
                    ?>
